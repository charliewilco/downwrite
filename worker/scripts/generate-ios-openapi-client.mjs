...
const outputPath = path.resolve(
  workerDirectory,
  "../iOS/Downwrite/Downwrite/Generated/DownwriteAPI.swift",
);

const spec = createOpenApiDocument("http://localhost:8787/api/v1/openapi.json");
const operations = Object.values(spec.paths).flatMap((methods) =>
  Object.values(methods).map((operation) => operation.operationId),
);

const header = `// Generated from Downwrite OpenAPI ${spec.openapi}.
// Source operation IDs: ${operations.filter(Boolean).join(", ")}.
`;
...
console.log(
  `Generated ${path.relative(process.cwd(), outputPath)} from ${operations.length} OpenAPI operations.`,
);
