...
- Single-Worker deployment shape: `curl http://localhost:8787/` returned compiled Preact `index.html`; `curl http://localhost:8787/workspaces/not-real` returned the SPA fallback; `curl http://localhost:8787/api/v1/health` returned `{"ok":true,"name":"downwrite","version":"v1"}`; `curl http://localhost:8787/.well-known/downwrite` returned discovery metadata with `baseUrl` on the same `http://localhost:8787` origin.
