...
- Workspace directory: verified `http://localhost:8787/` served by the Worker web path, with one demo workspace, one intended demo document, bootstrap-required rail state, and workspace creation controls.
...
- Single-Worker deployment shape: `curl http://localhost:8787/` returned the web app HTML; `curl http://localhost:8787/workspaces/not-real` returned a browser route response; `curl http://localhost:8787/api/v1/health` returned `{"ok":true,"name":"downwrite","version":"v1"}`; `curl http://localhost:8787/.well-known/downwrite` returned discovery metadata with `baseUrl` on the same `http://localhost:8787` origin.
