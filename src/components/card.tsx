    ...
    <div className="shadow-md dark:bg-onyx-800 p-2" data-testid="CARD">
      <header className="border-b-2 dark:border-onyx-600 pb-2 mb-12">
        <h2 className="font-bold text-base leading-none" data-testid="CARD_TITLE">
          <EditLink title={props.title} id={props.id} className="dark:text-white" />
        </h2>
        {props.dateAdded && (
          <small className="opacity-50 text-xs dark:text-white">
      ...
      <footer className="text-xs flex font-bold justify-between">
        <div data-testid="CARD_EXCERPT">
          <EditLink className="dark:text-white mr-2" id={props.id} />
          {props.public && (
            <PreviewLink className="dark:text-white mr-2" id={props.id} />
          )}
        </div>
        {props.onDelete && (
          <button
